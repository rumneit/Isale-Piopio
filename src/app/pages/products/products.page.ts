...
export class ProductsPage implements OnInit {
  ...
  search = '';
  /** ISale dropdown "Lọc theo": Tất cả | Còn Hạn SD | Gần đây | Còn số lượng (mặc định: Gần đây) */
  readonly chip = signal<'all' | 'instock' | 'notexpired' | 'recent'>('recent');
  ...
  /** ISale: gear icon — cấu hình chung */
  ...
  async selectChip(chip: 'all' | 'instock' | 'notexpired' | 'recent') {
    this.chip.set(chip);
    this.page.set(1);
    await this.load();
  }
